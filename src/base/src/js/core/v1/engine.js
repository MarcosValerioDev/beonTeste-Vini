...
engine.prototype.getSourceHost = function(format) {
    ...
    if (!tag) {
        console.warn("cannot locate source tag");
        return false;
    }
...
engine.prototype.create = function() {
    ...
    // set headers
    this.transport().setHeader("x-beon-client", client);
    this.transport().setHeader("x-beon-token", token);
    ...
    this.transport().setHeader("beon-session-id", this.session_id);
    this.transport().setHeader("beon-customer-id", this.customer_id);
