...
buddy.prototype.transport = function(api) {
    var instance = this.engine.transport(true);

    if (!this.api.hasOwnProperty(api))
        throw new Error("Invalid api endpoint: " + api);

    instance.host = this.api.content;
...
buddy.prototype.handleRegions = function(regions) {
    ...
    this.engine.dispatchEvent(
        "buddy.regions.handle.collection.before",
        regions
    );
...
buddy.prototype.injectRegion = function(region) {
    ...
    container.addEventListener(
        ...
        function() {
            this.engine.dispatchEvent(
                "buddy.regions.handle.item.inject.success",
                {
                    region: region
                }
            );
...
buddy.prototype.handleInteraction = function(event) {
    ...
    if (anchor != null) {
        ...
        interaction.product_id = interactionItem.getAttribute(
            "data-product-id"
        );
